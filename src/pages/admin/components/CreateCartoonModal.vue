      ...
      >
        ...
        <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
          ...
          >
            ...
            />
            {{ isCreating ? 'Creating...' : 'Create Cartoon' }}
          </Button>
...
import { useCreateAdminCartoon } from '../queries/useAdminContent'
import { XMarkIcon, PlusIcon, ArrowPathIcon } from '@heroicons/vue/24/outline'
import type { AdminCartoonCreate } from '../types'
...
interface Emits {
  (e: 'close'): void
  (e: 'created', cartoon: { id: number; title: string; link: string }): void
}

defineProps<Props>()
const emit = defineEmits<Emits>()

const createCartoon = useCreateAdminCartoon()

// Computed properties for proper reactivity
const isCreating = computed(() => createCartoon.isPending.value)

const form = reactive<AdminCartoonCreate>({
  title: '',
  link: '',
})
...
const handleSubmit = async () => {
  ...
  try {
    const result = await createCartoon.mutateAsync({
    ...
    console.error('Failed to create cartoon:', error)
  }
}
</script>
